...
//
    ...
    void testScoreBoost() {
        // tests popping multiple regions at once. n regions = 2^n points
        Shape lineShape = new Shape(List.of(
                new Cell(0, 0), new Cell(1, 0), new Cell(2, 0)
        ));
        Piece piece = new Piece(lineShape, new Cell(1, 0));
        model.place(piece);
        assertEquals(0, model.getScore(), "Score calculation depends on poppable regions and should start at 0.");
        // now place enough pieces to pop a region and check the score is 8
        model.place(new Piece(lineShape, new Cell(1, 1)));
        model.place(new Piece(lineShape, new Cell(1, 2)));
        model.place(new Piece(lineShape, new Cell(1, 3)));
        model.place(new Piece(lineShape, new Cell(1, 4)));
        model.place(new Piece(lineShape, new Cell(1, 5)));
        model.place(new Piece(lineShape, new Cell(1, 6)));
        model.place(new Piece(lineShape, new Cell(1, 7)));
        model.place(new Piece(lineShape, new Cell(1, 8)));
        assertEquals(8, model.getScore(), "Score should be 8 after popping a region: " + model.getScore());
